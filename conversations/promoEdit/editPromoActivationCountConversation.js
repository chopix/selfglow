...
export const editPromoActivationCountConversation = async (
	conversation,
	ctx
) => {
	try {
		const inline = new InlineKeyboard().text('Отмена', 'promoEditingBack')
		await ctx.reply(`Введите новое кол-во активаций тарифа`, {
			reply_markup: inline,
		})
		...
		if (update.update.callback_query) {
			...
			await Promocode.update(
				{ activationCount: update.update.message.text },
