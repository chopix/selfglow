...
export const editPromoActivationCountPerUserConversation = async (
	conversation,
	ctx
) => {
	try {
		const inline = new InlineKeyboard().text('Отмена', 'promoEditingBack')
		await ctx.reply(`Введите новое кол-во активаций тарифа на 1 человека`, {
			reply_markup: inline,
		})
		...
		if (update.update.callback_query) {
			...
			await Promocode.update(
				{ activationCountPerUser: update.update.message.text },
