...
export const banUserConversation = async (conversation, ctx) => {
	try {
		const inline = new InlineKeyboard().text('🔙 Назад', 'banBack').row()
		await ctx.reply(
			'Введите @username или ID пользователя, которого вы хотите забанить:',
			{
				reply_markup: inline,
			}
		)
		...
		return ctx.reply('Пользователь успешно забанен', { reply_markup: inline })
	} catch (e) {
		console.log(e)
	}
}
